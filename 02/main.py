...
def check_sample(levels):
    sorted_condition = sorted(levels)==levels or sorted(levels,reverse=True)==levels
    diff_condition = all(1<=abs(levels[i]-levels[i+1])<=3 for i in range(len(levels)-1))
...
def two(input):
    ...
    for levels in input:
        if check_sample(levels):
            ...
            if any(check_sample(levels[:i]+levels[i+1:]) for i in range(len(levels))):
                n_good+=1    
